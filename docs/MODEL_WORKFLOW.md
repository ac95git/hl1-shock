...
- **The SDK's split player meshes are broken exports; the whole-body one is sound.** Found 2026-09-19,
  the first maddened miner in game folded at the shoulders: `Player Models/player/player_template_biped(No_Head)1.smd`
  has its vertices on the ends of bone chains (the torso on `Spine2`, the hands on `Finger02`, nothing on
...
| `maddened_build.py [--no-compile] [--install]` | The maddened miner (2026-09-19): the player's body from the SDK sources as a monster. Copies the whole-body reference, the two heads, `reference_crowbar` and twelve animation SMDs; strips the head triangles from the body (the split "No_Head" file is broken, above); washes the suit's thirteen shell textures to a work blue as a second skin family (overalls); takes the black crowbar texture from `models/src/w_pickaxe/`; writes the QC with the player's renames, attachments, controllers and hitboxes, bodygroups head (bare, helmet) and weapon (blank, pick), the origin at the feet, and the monster's sequences with their activities (the crowbar swings as `ACT_MELEE_ATTACK1` with event 1 on frame 5); studiomdl. The pattern for a monster on the player's rig. |
