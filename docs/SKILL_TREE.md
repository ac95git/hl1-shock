...
| Pulse Discharge | 16 | Negated hits vent at the crosshair, as energy | — | Exists. **To be retired** (2026-09-23, [ADR-0016](adr/0016-the-discharge-is-innate-and-answers-only-slave-beams.md)): the Discharge becomes innate; id 16 is never reused and the node needs a new occupant |
...
| Headhunter | 38 | Hits to the head ×1.5 on top of the head multiplier, player hits only. Decapitation keys on the same hitgroup. **Since 2026-09-17 also ignores the grunt's helmet** (`CHGrunt::TraceAttack`), the one head armour in the roster, so the Stealth loop's silenced headshot lands — see [Stealth](#stealth) | Built; the helmet skip 2026-09-17, untested |
...
| **Shroud** | 160 | A flat ×0.8 on the fill for the holder, `skill_shroud_scale` in `PlayerConcealmentScale` — the ten roads over again in one node, so the Status page's Concealment line already includes it. Takes Cut the Head's cell (13,2). A stillness term was rejected because it rewards waiting, which no Stealth node does; a crouch-only boost was the second choice. Name provisional | **Built 2026-09-17**, untested in game |
| **Major: Silent Kill** | 130 | A kill on a monster below Spotted is **unheard**: no Disturbance is inserted (`PerceptionOnKilled`), so a squadmate around the corner never knows and nobody searches. **It is about the ears only** — a squadmate in sight is a witness in full, because soldiers are not blind, and the weapon's own noise is the weapon's. The play is stab, then drop the witness with a silenced headshot before its 0.75 fills. Settled 2026-09-17; the first text said "no witnesses" too | **Built 2026-09-17** with 5f, untested in game |
