...
radiation tile. Cleave's **ready icon**
...
| The Cleave wall impact | `weapons/cbar_hit1.wav` at pitch 60, once per wave where the bow first meets a wall | The crowbar's own wall hit, lower. Wants the sound of displaced air stopping, per weapon: a dull thump for the crowbar, a crackle for the katana |
| Cleave hitting a monster or entity | `weapons/cbar_hitbod1.wav` at pitch 85 where the bow meets it, client-side, rate-limited to one per 60 ms | The crowbar's body hit, lower, and the same for a crate as for a zombie. Wants a per-material impact, and per weapon: air breaking on flesh for the crowbar, energy discharging into it for the katana |
