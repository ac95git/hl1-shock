...
The read-back for a hand-edited map. Andrei builds in J.A.C.K.; this closes the loop from the
terminal without him leaving the editor. Never edit the `.map` while J.A.C.K. is open, and never run
`greybox.py` on a map J.A.C.K. has saved.
...
1. **If the diff shows nothing changed**, he saved the `.jmf` but did not export. Ctrl+S in J.A.C.K.
   writes `E:\Projects\jack\<name>.jmf`; the repo's `.map` changes only on File → Export. Say so and
   stop.
...
After his yes: commit the exported `.map` (never the `.bsp`), and add the session's entry to
`docs/CRAFT_LOG.md` if it taught something.
